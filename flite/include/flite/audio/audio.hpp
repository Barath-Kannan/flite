...
typedef struct cst_audiodev_struct {
    ...
    //FIXME: Converted to int from void* - make sure this is safe
    int platform_data;
} cst_audiodev;
...
int play_wave_sync(cst_wave* w, cst_relation* rel, int (*call_back)(cst_item*));
...
cst_audiodev* audio_open_file(int sps, int channels, cst_audiofmt fmt, const char* filename);
...
typedef struct cst_audio_streaming_info_struct {
    int min_buffsize;
    int (*asc)(const cst_wave* w, int start, int size, int last, struct cst_audio_streaming_info_struct* asi);
...
typedef int (*cst_audio_stream_callback)(const cst_wave* w, int start, int size, int last, cst_audio_streaming_info* asi);
