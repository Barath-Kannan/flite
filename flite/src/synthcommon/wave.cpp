...
void cst_wave_resize(cst_wave* w, int samples, int num_channels)
{
    ...
    if (num_channels == w->num_channels)
        memmove(ns, w->samples, sizeof(short) * num_channels * (samples < w->num_samples ? samples : w->num_samples));
    cst_free(w->samples);
    w->samples = ns;
...
cst_wave* concat_wave(cst_wave* dest, const cst_wave* src)
{
    ...
    cst_wave_resize(dest, dest->num_samples + src->num_samples, dest->num_channels);
    memcpy(dest->samples + orig_nsamps, src->samples, src->num_samples * src->num_channels * sizeof(short));
