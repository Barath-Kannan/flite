...
#if CLUNITS_DEBUG > 0
#define DPRINTF(l, x) \
    if (CLUNITS_DEBUG > (l)) cst_dbgmsg x
#else
#define DPRINTF(l, x)
#endif
...
static cst_vit_path* cl_path(cst_vit_path* p, cst_vit_cand* c, cst_viterbi* vd);
...
static int optimal_couple_frame(cst_clunit_db* cludb, int u0, int u1, cst_distfunc dfunc, int bestsofar);
static int optimal_couple(cst_clunit_db* cludb, int u0, int u1, int* u0_move, int* u1_move, cst_distfunc dfunc);
static int frame_distance(const cst_clunit_db* cludb, int a, int b, const int* join_weights, int order, int best);
static int frame_distanceb(const cst_clunit_db* cludb, int a, int b, const int* join_weights, int order, int best);
...
cst_utterance* clunits_dump_units(cst_utterance* utt)
{
    ...
    for (s = relation_head(utt_relation(utt, "Segment")); s; s = item_next(s)) {
        ...
        cst_dbgmsg("for %s end %f selected %d %s start move %d end move %d\n", item_name(s), item_feat_float(s, "end"), unit_entry, item_name(u), item_feat_int(u, "unit_start") - clunit_db->units[unit_entry].start, item_feat_int(u, "unit_end") - clunit_db->units[unit_entry].end);
    }
...
static cst_utterance* clunits_select(cst_utterance* utt)
{
    ...
    for (s = relation_head(segs); s; s = item_next(s)) {
        ...
        DPRINTF(0, ("selected %d=%s_%d %d %d\n", unit_entry, UNIT_TYPE(clunit_db, unit_entry), UNIT_INDEX(clunit_db, unit_entry), item_feat_int(u, "unit_start"), item_feat_int(u, "unit_end")));

        item_set_int(u, "target_end", (int)(item_feat_float(s, "end") * clunit_db->sts->sample_rate));
    }
...
static cst_vit_cand* cl_cand(cst_item* i, cst_viterbi* vd)
{
    ...
    if ((clunit_db->extend_selections > 0) && (item_prev(i))) {
        ...
        for (e = 0; lc && (e < clunit_db->extend_selections); lc = lc->next) {
            ...
            if ((gt == 0) && (clunit_db->units[nu].type == clunit_db->units[all->ival].type)) {
                p = new_vit_cand();
                p->next = all;
                p->item = i;
                p->score = 0;
...
static int optimal_couple_frame(cst_clunit_db* cludb, int u0, int u1, cst_distfunc dfunc, int bestsofar)
{
    ...
    return (*dfunc)(cludb, a, b, cludb->join_weights, cludb->mcep->num_channels, bestsofar) + abs(get_frame_size(cludb->sts, a) - get_frame_size(cludb->sts, b)) * cludb->f0_weight;
}
...
static int optimal_couple(cst_clunit_db* cludb, int u0, int u1, int* u0_move, int* u1_move, cst_distfunc dfunc)
{
    ...
    DPRINTF(1, ("optimal_coupling %s_%d (%d,%d) %s_%d (%d,%d)\n", UNIT_TYPE(cludb, u0), UNIT_INDEX(cludb, u0), cludb->units[u0].start, cludb->units[u0].end, UNIT_TYPE(cludb, u1), UNIT_INDEX(cludb, u1), cludb->units[u1].start, cludb->units[u1].end));
    ...
    DPRINTF(1, ("%s == %s, sliding u0:(%d,%d) u1:(%d,%d)\n", UNIT_TYPE(cludb, u0), UNIT_TYPE(cludb, u1_p), u0_st, u0_end, u1_p_st, u1_p_end));
    ...
    for (i = 0; i < fcount; ++i) {
        ...
        dist =
            (*dfunc)(cludb, a, b, cludb->join_weights, cludb->mcep->num_channels, best_val) + abs(get_frame_size(cludb->sts, a) - get_frame_size(cludb->sts, b)) * cludb->f0_weight;
...
static int frame_distance(const cst_clunit_db* cludb, int a, int b, const int* join_weights, int order, int bestsofar)
{
    int r, diff;
    int i;
    ...
    cst_dbgmsg("a(%d): ", a);
    for (i = 0; i < order; ++i)
        cst_dbgmsg("%.2f ", (double)av[i] * cludb->mcep->coeff_range / 65536 + cludb->mcep->coeff_min);
    cst_dbgmsg("\n");
    cst_dbgmsg("b(%d): ", b);
    for (i = 0; i < order; ++i)
        cst_dbgmsg("%.2f ", (double)bv[i] * cludb->mcep->coeff_range / 65536 + cludb->mcep->coeff_min);
    cst_dbgmsg("\n");
#endif
...
static int frame_distanceb(const cst_clunit_db* cludb, int a, int b, const int* join_weights, int order, int bestsofar)
{
    int r, diff;
    int i;
    ...
    cst_dbgmsg("a(%d): ", a);
    for (i = 0; i < order; ++i)
        cst_dbgmsg("%.2f ", (double)av[i] * cludb->mcep->coeff_range / 256 + cludb->mcep->coeff_min);
    cst_dbgmsg("\n");
    cst_dbgmsg("b(%d): ", b);
    for (i = 0; i < order; ++i)
        cst_dbgmsg("%.2f ", (double)bv[i] * cludb->mcep->coeff_range / 256 + cludb->mcep->coeff_min);
    cst_dbgmsg("\n");
#endif
