...
/* http://www.mvps.org/directx/articles/ */
{
    ...
    q = (0.5 * ((2.0 * p1) + (p * (-p0 + p2)) + ((p * p) * (((2.0 * p0) - (5.0 * p1)) + ((4.0 * p2) - p3))) + ((p * p * p) * (-p0 + ((3.0 * p1) - (3.0 * p2)) + p3))));
    /*    (set! q (* 0.5 (+ (* 2 p1)
           (* (+ (* -1 p0) p2) p)
...
static void cg_F0_interpolate_spline(cst_utterance* utt, cst_track* param_track)
{
    ...
    for (syl = utt_rel_head(utt, "Syllable"); syl; syl = item_next(syl)) {
        ...
        if (item_next(syl)) /* not last syllable */
            end_f0 = (param_track->frames[end_index - 1][0] +
                      param_track->frames[end_index][0]) /
                     2.0;
        else
            end_f0 = param_track->frames[end_index - 1][0];
...
static void cg_smooth_F0(cst_utterance* utt, cst_cg_db* cg_db, cst_track* param_track)
{
    ...
    for (i = 0, mcep = utt_rel_head(utt, "mcep"); mcep; i++, mcep = item_next(mcep)) {
        if (voiced_frame(mcep)) {
            ...
            param_track->frames[i][0] =
                (((param_track->frames[i][0] - cg_db->f0_mean) / cg_db->f0_stddev) * stddev) + mean;
            /* Some safety checks */
    ...
    else /* let's always do this second one in case model_shape isn't set */
    {
        for (i = 0; i < cg_db->num_channels[pm]; i++) {
            v[i] = cg_db->model_min[i] +
                   ((float)((cg_db->model_vectors[pm][f][i]) / 65535.0) * cg_db->model_range[i]);
        }
        return 0;
    }
}
...
static cst_utterance* cg_predict_params(cst_utterance* utt)
{
    ...
    cst_track_resize(param_track, utt_feat_int(utt, "param_track_num_frames"), (cg_db->num_channels[0] / fff) - (2 * extra_feats)); /* no voicing or str */
...
static cst_utterance* cg_resynth(cst_utterance* utt)
{
    ...
    if (cg_db->do_mlpg) {
        ...
        w = mlsa_resynthesis(smoothed_track, str_track, cg_db, asi, mlsa_speed_param);
        delete_track(smoothed_track);
    }
    else
        w = mlsa_resynthesis(param_track, str_track, cg_db, asi, mlsa_speed_param);
