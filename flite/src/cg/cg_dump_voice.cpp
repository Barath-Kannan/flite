...
static void cst_cg_write_tree_nodes(cst_file fd, const cst_cart_node* nodes)
{
    ...
    for (i = 0; i < num_nodes; i++) {
        ...
        if (nodes[i].val->c.a.type == CST_VAL_TYPE_STRING) {
            cst_cg_write_padded(fd, nodes[i].val->c.a.v.vval, cst_strlen(nodes[i].val->c.a.v.vval) + 1);
        }
        ...
        else { /* its not going to work without more code ... */
...
static void cst_cg_write_2d_array_short(cst_file fd, const unsigned short** data, int rows, int cols)
{
    int i;
...
static void cst_cg_write_2d_array_float(cst_file fd, const float* const* data, int rows, int cols)
{
    int i;
...
static void cst_cg_write_2d_array_double(cst_file fd, const double* const* data, int rows, int cols)
{
    int i;
...
static void cst_cg_write_voice_feature(cst_file fd, const char* fname, const char* fval)
{
...
int cst_cg_dump_voice(const cst_voice* v, const cst_string* filename)
{
    ...
    cst_cg_write_voice_feature(fd, "language", get_param_string(v->features, "language", "eng"));
    if (get_param_string(v->features, "secondary_languages", NULL))
        cst_cg_write_voice_feature(fd, "secondary_languages", get_param_string(v->features, "secondary_languages", ""));
    cst_cg_write_voice_feature(fd, "country", get_param_string(v->features, "country", "USA"));
    cst_cg_write_voice_feature(fd, "variant", get_param_string(v->features, "variant", "none"));
    cst_cg_write_voice_feature(fd, "age", get_param_string(v->features, "age", "30"));
    cst_cg_write_voice_feature(fd, "gender", get_param_string(v->features, "gender", "unknown"));
    cst_cg_write_voice_feature(fd, "build_date", get_param_string(v->features, "build_date", "unknown"));
    cst_cg_write_voice_feature(fd, "description", get_param_string(v->features, "description", "unknown"));
    if (get_param_string(v->features, "prompt_dur", NULL))
        cst_cg_write_voice_feature(fd, "prompt_dur", get_param_string(v->features, "prompt_dur", "unknown"));
    if (get_param_string(v->features, "english_data", NULL))
        cst_cg_write_voice_feature(fd, "english_data", get_param_string(v->features, "english_data", "unknown"));
    ...
    if (get_param_string(v->features, "eng_shared", NULL))
        cst_cg_write_voice_feature(fd, "eng_shared", get_param_string(v->features, "eng_shared", "0"));
    ...
    if (get_param_string(v->features, "int_f0_target_mean", NULL))
        cst_cg_write_voice_feature(fd, "int_f0_target_mean", get_param_string(v->features, "int_f0_target_mean", NULL));
    if (get_param_string(v->features, "int_f0_target_stddev", NULL))
        cst_cg_write_voice_feature(fd, "int_f0_target_stddev", get_param_string(v->features, "int_f0_target_stddev", NULL));
    if (get_param_string(v->features, "duration_stretch", NULL))
        cst_cg_write_voice_feature(fd, "duration_stretch", get_param_string(v->features, "duration_stretch", NULL));

    cst_cg_write_voice_feature(fd, "copyright", get_param_string(v->features, "copyright", "unknown"));

    /* These features define the number of models to be read */
    cst_cg_write_voice_feature(fd, "num_dur_models", val_string(val_string_n(db->num_dur_models)));
    cst_cg_write_voice_feature(fd, "num_param_models", val_string(val_string_n(db->num_param_models)));
    cst_cg_write_voice_feature(fd, "model_shape", val_string(val_string_n(db->model_shape)));
    cst_cg_write_voice_feature(fd, "num_f0_models", val_string(val_string_n(db->num_f0_models)));
    ...
    for (pm = 0; pm < db->num_param_models; pm++) {
        ...
        cst_cg_write_2d_array_short(fd, db->model_vectors[pm], db->num_frames[pm], cst_actual_num_channels(db, pm));
    }

    if (db->spamf0) {
        cst_fwrite(fd, &db->num_channels_spamf0_accent, sizeof(int), 1);
        cst_fwrite(fd, &db->num_frames_spamf0_accent, sizeof(int), 1);
        cst_cg_write_2d_array_float(fd, db->spamf0_accent_vectors, db->num_frames_spamf0_accent, db->num_channels_spamf0_accent);
    }
    ...
    if (db->model_shape > CST_CG_MODEL_SHAPE_BASE_MINRANGE) { /* dump the qtable if shape > 1 */
        for (pm = 0; pm < db->num_param_models; pm++) {
            cst_cg_write_2d_array_float(fd, db->qtable[pm], db->num_channels[pm], /* "frames" */
                                        256);
        }
    }
    ...
    cst_cg_write_2d_array_double(fd, db->me_h, db->ME_num, db->ME_order);
