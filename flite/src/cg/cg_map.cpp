...
cst_cg_db* cst_cg_load_db(cst_voice* vox, cst_file fd, int bs /* byteswapped */)
{
    ...
    db->model_shape = get_param_int(vox->features, "model_shape", CST_CG_MODEL_SHAPE_BASE_MINRANGE);
    ...
    if (db->model_shape > CST_CG_MODEL_SHAPE_BASE_MINRANGE) { /* there is a qtable if shape > 1 */
