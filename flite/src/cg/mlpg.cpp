...
static double get_like_pdfseq_vit(int dim, int dim2, int dnum, int clsnum, MLPGPARA param, float** model, XBOOL dia_flag)
{
    ...
    for (d = 0, like = 0.0; d < dnum; d++) {
        ...
        if (dia_flag == XTRUE) {
            ...
            sumgauss = get_gauss_dia(0, param->ov, param->clsdetv, param->wght, param->mean, param->clscov);
        }
        else {
            ...
            sumgauss = get_gauss_full(0, param->ov, param->clsdetv, param->wght, param->mean, param->clscov);
        }
        if (sumgauss <= 0.0)
                             ...
                             DMATRIX invcovmat)        /* [clsnum * dim][dim] */
{
    ...
    gauss = weightmat->data[clsidx][0] / sqrt(pow(2.0 * PI, (double)vec->length) * detvec->data[clsidx]) * exp(-1.0 * cal_xmcxmc(clsidx, vec, meanvec, invcovmat) / 2.0);
                            ...
                            DMATRIX invcovmat)        /* [clsnum][dim] */
{
    ...
    gauss = weightmat->data[clsidx][0] / sqrt(pow(2.0 * PI, (double)vec->length) * detvec->data[clsidx]) * exp(-gauss / 2.0);
...
static void pst_free(PStreamChol* pst)
{
    ...
    mlpg_free(pst->dw.width);
    pst->dw.width = NULL;
    for (i = 0; i < pst->dw.num; i++)
        mlpg_free(pst->dw.coef_ptrs[i]);
    mlpg_free(pst->dw.coef);
    pst->dw.coef = NULL;
    mlpg_free(pst->dw.coef_ptrs);
    pst->dw.coef_ptrs = NULL;
...
cst_track* mlpg(const cst_track* param_track, cst_cg_db* cg_db)
{
    ...
    get_like_pdfseq_vit(dim, dim_st, nframes, nframes, param, param_track->frames, XTRUE);
