...
static void cst_synthtilt(const cst_cg_db* cg_db, const float start, const float peak, const float tiltamp, const float tiltdur, const float tilttilt, cst_track* ftrack);
...
cst_utterance* cst_spamf0(cst_utterance* utt)
{
    ...
    cst_track_resize(spamf0_track, (utt_feat_int(utt, "param_track_num_frames")), 1);
    ...
    for (s = utt_rel_head(utt, "Syllable"); s; s = item_next(s)) {
        ...
        syldur = ffeature_float(s, "R:SylStructure.daughtern.R:Segment.end") - ffeature_float(s, "R:SylStructure.daughter1.R:Segment.p.end");
        cst_synthtilt(cg_db, ffeature_float(s, "R:SylStructure.daughter1.R:Segment.p.end"), cg_db->spamf0_accent_vectors[f][0], cg_db->spamf0_accent_vectors[f][2], syldur, cg_db->spamf0_accent_vectors[f][6], spamf0_track);
    }
...
void cst_synthtilt(const cst_cg_db* cg_db, const float start, const float peak, const float tiltamp, const float tiltdur, const float tilttilt, cst_track* ftrack)
{
