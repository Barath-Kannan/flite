...
static cst_val* lex_lookup_addenda(const char* wp, const cst_lexicon* l, int* found);

static int lex_match_entry(const char* a, const char* b);
static int lex_lookup_bsearch(const cst_lexicon* l, const char* word);
static int find_full_match(const cst_lexicon* l, int i, const char* word);
...
{ /* if replace then replace entry in addenda of lex with entry */
    ...
    ventry = cons_val(string_val(word), cons_val(string_val(pos), val_reverse(phones)));
    cst_free(word);
    cst_free(pos);
    ts_close(e);
#if 0
...
int in_lex(const cst_lexicon* l, const char* word, const char* pos, const cst_features* feats)
{
...
cst_val* lex_lookup(const cst_lexicon* l, const char* word, const char* pos, const cst_features* feats)
{
    int index;
    int p;
    const char* q;
    ...
    if (!found) {
        ...
        if (index >= 0) {
            if (l->phone_hufftable) {
                for (p = index - 2; l->data[p]; p--)
                    for (q = l->phone_hufftable[l->data[p]]; *q; q++)
                        phones = cons_val(string_val(l->phone_table[(unsigned char)*q]), phones);
            }
            else /* no compression -- should we still support this ? */
            {
                for (p = index - 2; l->data[p]; p--)
                    phones = cons_val(string_val(l->phone_table[l->data[p]]), phones);
            }
            ...
            phones = lts_apply(word, "", /* more features if we had them */
                               l->lts_rule_set);
        }
    }
...
static int lex_uncompress_word(char* ucword, int max_size, int p, const cst_lexicon* l)
{
    int i, j = 0, length;
    unsigned char* cword;
