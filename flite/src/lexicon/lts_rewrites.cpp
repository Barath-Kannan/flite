#include "flite/lexicon/lts_rewrites.hpp"

#include "flite/utils/string.hpp"
#include "flite/utils/val.hpp"

static int item_match(const cst_val* PATT, const cst_val* THING, const cst_val* sets)
{
    const cst_val* sss;
    ...
    for (sss = sets; sss; sss = val_cdr(sss)) {
        if (cst_streq(val_string(val_car(val_car(sss))), val_string(PATT))) { /* Its a set not a letter */
...
static int context_match(const cst_val* PATTERN, const cst_val* STRING, const cst_val* sets)
{
    int r, s, t;
    ...
    if (!PATTERN)
        ...
        r = FALSE;
    else if (val_cdr(PATTERN) && (cst_streq("*", val_string(val_car(PATTERN))))) {
...
static int rule_matches(const cst_val* LC, const cst_val* RC, const cst_val* RLC, const cst_val* RA, const cst_val* RRC, const cst_val* sets)
{
    const cst_val *rc, *ra;
...
static const cst_val* find_rewrite_rule(const cst_val* LC, const cst_val* RC, const cst_lts_rewrites* r)
{
