...
int audio_close_oss(cst_audiodev* ad)
{
    ...
    ioctl(ad->platform_data, SNDCTL_DSP_SYNC, NULL);
    rv = close(ad->platform_data);
    cst_free(ad);
    return rv;
}
...
{
    return write(ad->platform_data, samples, num_bytes);
}

int audio_flush_oss(cst_audiodev* ad)
{
    return ioctl(ad->platform_data, SNDCTL_DSP_SYNC, NULL);
}

int audio_drain_oss(cst_audiodev* ad)
{
    return ioctl(ad->platform_data, SNDCTL_DSP_RESET, NULL);
}
