...
#define DPRINTF(l, x) \
    if (SYNTH_MODULES_DEBUG > l) cst_dbgmsg x
#else
#define DPRINTF(l, x)
#endif
...
cst_utterance* default_tokenization(cst_utterance* u)
{
    ...
    fd = ts_open_string(text, get_param_string(u->features, "text_whitespace", NULL), get_param_string(u->features, "text_singlecharsymbols", NULL), get_param_string(u->features, "text_prepunctuation", NULL), get_param_string(u->features, "text_postpunctuation", NULL));
...
cst_utterance* default_textanalysis(cst_utterance* u)
{
    ...
    for (t = relation_head(utt_relation(u, "Token")); t; t = item_next(t)) {
        ...
        for (w = words; w; w = val_cdr(w)) {
            ...
            if (cst_val_consp(val_car(w))) { /* Has extra features */
                item_set_string(word, "name", val_string(val_car(val_car(w))));
                feat_copy_into(val_features(val_cdr(val_car(w))), item_feats(word));
            }
            else
...
cst_utterance* cart_intonation(cst_utterance* u)
{
    ...
    for (s = relation_head(utt_relation(u, "Syllable")); s; s = item_next(s)) {
        ...
        DPRINTF(0, ("word %s gpos %s stress %s ssyl_in %s ssyl_out %s accent %s endtone %s\n", ffeature_string(s, "R:SylStructure.parent.name"), ffeature_string(s, "R:SylStructure.parent.gpos"), ffeature_string(s, "stress"), ffeature_string(s, "ssyl_in"), ffeature_string(s, "ssyl_out"), ffeature_string(s, "accent"), ffeature_string(s, "endtone")));
    }
...
cst_utterance* cart_duration(cst_utterance* u)
{
    ...
    for (s = relation_head(utt_relation(u, "Segment")); s; s = item_next(s)) {
        ...
        local_dur_stretch = ffeature_float(s,
                                           "R:SylStructure.parent.parent."
        ...
        DPRINTF(0, ("phone %s accent %s stress %s pdur %f stretch %f mean %f std %f dur %f\n", item_name(s), ffeature_string(s, "R:SylStructure.parent.accented"), ffeature_string(s, "R:SylStructure.parent.stress"), zdur, local_dur_stretch, dur_stat->mean, dur_stat->stddev, dur));
        end += dur;
    ...
    for (word = relation_head(utt_relation(u, "Word"));
        ...
        if (item_feat_present(item_parent(item_as(word, "Token")), "phones")) {
            ...
            if (cst_val_consp(vpn)) { /* for SAPI ?? */
                ...
                dp = 1;
                if (cst_streq(val_string(vpn), ffeature_string(word, "p.R:Token.parent.phones")))
            ...
            if (wp)
                ...
                dp = 1;
                phones = lex_lookup(lex, item_feat_string(word, "name"), pos, u->features);
            }
        }
...
int default_utt_break(cst_tokenstream* ts, const char* token, cst_relation* tokens)
{
    ...
    else if (((cst_streq(ltoken, "Yahoo")) || (cst_streq(ltoken, "YAHOO")) || (cst_streq(ltoken, "yahoo"))) && strchr(postpunct, '!') && strchr("abcdefghijklmnopqrstuvwxyz", token[0]))
        return FALSE;
    else if (strchr(postpunct, ':') || strchr(postpunct, '?') || strchr(postpunct, '!'))
        return TRUE;
    else if (strchr(postpunct, '.') && (cst_strlen(ts->whitespace) > 1) && strchr("ABCDEFGHIJKLMNOPQRSTUVWXYZ", token[0]))
        return TRUE;
             ...
             !(strchr("ABCDEFGHIJKLMNOPQRSTUVWXYZ", ltoken[cst_strlen(ltoken) - 1]) || ((cst_strlen(ltoken) < 4) && strchr("ABCDEFGHIJKLMNOPQRSTUVWXYZ", ltoken[0]))))
        return TRUE;
    else
        return FALSE;
}
