...
cst_voice* register_cmu_us_slt(const char* voxdir, cst_voice* vox)
{
    cst_lexicon* lex;
    vox->name = "slt";
...
cst_voice* register_cmu_us_slt(const char* voxdir)
{
    cst_voice* vox;
    vox = new_voice();
    return register_cmu_us_slt(voxdir, vox);
}

namespace flite {

voice make_us_slt(std::string_view voxdir)
{
    voice v;
    const auto s = std::string{voxdir};
    register_cmu_us_slt(s.c_str(), v.operator->());
    return v;
}
