...
static void apply_lr_model(cst_item* s, const us_f0_lr_term* f0_lr_terms, float* start, float* mid, float* end)
{
    int i;
    const cst_val* v = 0;
    float fv;
...
{
    if ((item_prev(syl) == 0) ||
        (cst_streq("pau", ffeature_string(syl, "R:SylStructure.daughter.R:Segment.p.name"))))
        return TRUE;
    else
        return FALSE;
}
...
{
    if ((item_next(syl) == 0) ||
        (cst_streq("pau", ffeature_string(syl, "R:SylStructure.daughtern.R:Segment.n.name"))))
        return TRUE;
    else
        return FALSE;
}
...
static float vowel_mid(cst_item* syl)
{
    ...
    for (s = ts; s; s = item_next(s)) {
        if (cst_streq("+", phone_feature_string(ps, item_feat_string(s, "name"), "vc"))) {
            return (item_feat_float(s, "end") +
                    ffeature_float(s, "R:Segment.p.end")) /
                   2.0;
        }
    }
    ...
    return (item_feat_float(ts, "end") +
            ffeature_float(ts, "R:Segment.p.end")) /
           2.0;
}
...
cst_utterance* us_f0_model(cst_utterance* u)
{
...

    {
        ...
        add_target_point(targ_rel, ffeature_float(syl, "R:SylStructure.daughter.R:Segment.p.end"), map_f0((start + lend) / 2.0, local_mean, local_stddev));
        add_target_point(targ_rel, vowel_mid(syl), map_f0(mid, local_mean, local_stddev));
        lend = map_f0(end, local_mean, local_stddev);
        if (pre_break(syl))
            add_target_point(targ_rel, ffeature_float(syl, "R:SylStructure.daughtern.end"), map_f0(end, local_mean, local_stddev));
    }
